...
static void secp256k1_bppp_commit_initial_data(
    ...
    secp256k1_sha256_write(transcript, scratch, 8);
    secp256k1_bppp_le64(scratch, (uint64_t) num_proofs);
    secp256k1_sha256_write(transcript, scratch, 8);
    for (i = 0; i < num_proofs; i++) {
        secp256k1_bppp_le64(scratch, min_value[i]);
        secp256k1_sha256_write(transcript, scratch, 8);
        /* TODO: THIS IS INEFFICIENT, we are re-using assetgenp, fix this later*/
        secp256k1_bppp_serialize_points(scratch, &commitp[i], asset_genp);
